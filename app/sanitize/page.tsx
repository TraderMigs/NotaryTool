...
export default function SanitizePage() {
  ...
  const rectsHistoryRef = useRef<PageRectsMap[]>([]);
  ...
  const [additionalSignerFee, setAdditionalSignerFee] = useState<string>("2");
  const [addonFee, setAddonFee] = useState<string>("0");
  const [ronEnabled, setRonEnabled] = useState(false);
  const RON_FEE = 25;

  const feeTotal = useMemo(() => {
    const base = parseFloat(baseFee) || 0;
    const addlFee = parseFloat(additionalSignerFee) || 0;
    const addon = parseFloat(addonFee) || 0;
    const additionalSigners = Math.max(0, signerCount - 1);
    const ron = ronEnabled ? RON_FEE : 0;
    return base + (additionalSigners * addlFee) + addon + ron;
  }, [baseFee, signerCount, additionalSignerFee, addonFee, ronEnabled]);
  ...
  function handleExportReceipt() {
    ...
    lines.push("");
    lines.push("Generated by Specterfy. Session-only — not saved or transmitted.");
    ...
    const printContent = `
      ...
      <body>
        <h2>Fee Receipt</h2>
        <div class="sub">Specterfy — Session only. Not stored.</div>
        <pre>${lines.join("\n")}</pre>
      </body></html>
    `;
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(async ({ data }) => {
      ...
      try {
        ...
        setIsOwner(status.owner ?? false);
        setIsPaid(status.paid ?? false);
        setTodayCount(status.todayCount ?? 0);
        setLimitHit(status.limitHit ?? false);
      } catch {
        setIsOwner(false);
        setIsPaid(false);
      }
      ...
      return (
        <article key={page.pageNumber} style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '12px', overflow: 'hidden', marginBottom: inFullscreen ? '12px' : '14px' }}>
          ...
          >
            ...
            {rects.map((rect, idx) => (
              <div key={`${page.pageNumber}-${idx}`} style={{ position: 'absolute', background: '#000', opacity: 1, ...getRectStyle(rect), pointerEvents: 'none' }}>
                <div
                  title="Remove this box"
                  onMouseDown={(e) => { e.stopPropagation(); e.preventDefault(); removeRect(page.pageNumber, idx); }}
                  onTouchStart={(e) => { e.stopPropagation(); removeRect(page.pageNumber, idx); }}
                  style={{
                    position: 'absolute', top: '2px', right: '2px', width: '18px', height: '18px',
                    borderRadius: '50%', background: 'rgba(220,50,50,0.95)', display: 'flex',
                    alignItems: 'center', justifyContent: 'center', fontSize: '10px', fontWeight: 900,
                    color: '#fff', cursor: 'pointer', pointerEvents: 'auto', userSelect: 'none',
                    zIndex: 10, lineHeight: 1, boxShadow: '0 1px 3px rgba(0,0,0,0.4)',
                  }}
                >✕</div>
              </div>
            ))}
          ...
          <div className="sanitize-mobile-upload">
            ...
            {pagePreviews.length > 0 && (
              <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '12px', padding: '16px 18px', display: 'flex', justifyContent: 'space-between', gap: '8px' }}>
                {[{ val: pageCount, k: 'Pages' }, { val: totalRedactions, k: 'Redactions' }, { val: `$${feeTotal.toFixed(2)}`, k: 'Fee total' }].map(s => (
                  <div key={s.k} style={{ textAlign: 'center' }}>
                    <div style={{ fontFamily: 'var(--dm-sans, sans-serif)', fontSize: '20px', fontWeight: 700, color: s.k === 'Fee total' ? 'var(--cyan)' : 'var(--text-primary)', letterSpacing: '-0.02em' }}>{s.val}</div>
              ...
              <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '10px', padding: '18px 16px' }}>
                ...
                {[{ val: pageCount, key: 'Pages' }, { val: totalRedactions, key: 'Redactions' }, { val: `$${feeTotal.toFixed(2)}`, key: 'Fee total' }].map((s, i, arr) => (
                  <div key={s.key} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 0', borderBottom: i < arr.length - 1 ? '1px solid var(--border)' : 'none' }}>
                    <span style={{ fontFamily: 'var(--dm-sans, sans-serif)', fontSize: '17px', fontWeight: 700, color: s.key === 'Fee total' ? 'var(--cyan)' : 'var(--text-primary)', letterSpacing: '-0.015em' }}>{s.val}</span>
