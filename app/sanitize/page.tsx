  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(async ({ data }) => {
      ...
      try {
        ...
        // Fallback: treat as free with unknown count
        setIsOwner(false);
        setIsPaid(false);
      }
      setAuthChecked(true);
            ...
            {rects.map((rect, idx) => (
              <div
                ...
                style={{
                  ...
                  opacity: 1,
                  ...getRectStyle(rect),
                  // Box itself passes pointer events through so drawing still works
                  pointerEvents: 'none',
                }}
              >
                {/* X button — always visible, pointer-events re-enabled */}
                <div
                  title="Remove this box"
                  onMouseDown={(e) => {
                    e.stopPropagation();
                    e.preventDefault();
                    removeRect(page.pageNumber, idx);
                  }}
                  onTouchStart={(e) => {
                    e.stopPropagation();
                    removeRect(page.pageNumber, idx);
