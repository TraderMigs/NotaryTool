...
// Sends renewal reminder ~30 days before yearly plan renews
...
export async function POST(request: NextRequest) {
  ...
  if (error || !yearlyUsers?.length) return NextResponse.json({ sent: 0 })
  ...
  for (const row of yearlyUsers) {
    ...
    const daysUntil = Math.round((renewalDate.getTime() - Date.now()) / (1000 * 60 * 60 * 24))

    if (daysUntil >= 29 && daysUntil <= 31) {
      const { data: u } = await supabase.from('admin_user_summary').select('email').eq('id', row.user_id).single()
      if (u?.email) {
        await resend.emails.send({
          ...
          html: `<div style="font-family:sans-serif;background:#050C15;color:#fff;padding:40px;max-width:560px;margin:0 auto;border-radius:12px;">
            ...
            <p style="color:rgba(255,255,255,0.6);font-size:15px;line-height:1.7;margin-bottom:16px;">
              Your Specterfy yearly plan ($89/yr) renews on <strong style="color:#fff;">${renewalDate.toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</strong>.
            </p>
            <p style="color:rgba(255,255,255,0.6);font-size:15px;line-height:1.7;margin-bottom:24px;">
              To cancel before renewal, use the billing portal below.
            </p>
            <a href="https://billing.stripe.com/p/login/aFafZjeAjbwVgnN9wCafS00" style="display:inline-block;background:#00C8F0;color:#020C14;font-weight:600;font-size:14px;padding:12px 24px;border-radius:8px;text-decoration:none;">Manage billing</a>
            <p style="color:rgba(255,255,255,0.3);font-size:12px;margin-top:32px;line-height:1.6;">
              No action needed to continue. Specterfy is a privacy pre-processing utility — not a legal compliance platform.
            </p>
          </div>`,
        }).catch(() => {})
        sent++
